...
export const Futuristic3DBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 3. WebGL Renderer with optimized PixelRatio capping for smooth scrolling
    ...
    const gridWidth = isMobile ? 28 : 50;
    const gridDepth = isMobile ? 28 : 50;
    const gridSegments = isMobile ? 24 : 36; // Optimized grid density
    ...
    // 7. Mouse Interaction & Passive Scroll Performance Monitor
    let targetMouseX = 0;
    let targetMouseY = 0;
    let currentMouseX = 0;
    let currentMouseY = 0;
    ...
    // 9. Ultra-Smooth 60 FPS Render Loop (Optimized for zero scroll lag)
    ...
    const animate = () => {
      ...
      // Animate 3D Cyber Wave Grid Vertices (Throttled slightly during active scrolling to keep 60 FPS)
      ...
      for (let i = 0; i < count; i++) {
        ...
        posArr[i * 3 + 1] = wave1 + wave2;
      }
      posAttr.needsUpdate = true;

      // Rotate Starfield
      starSystem.rotation.y = elapsedTime * 0.01;

      // Update Neural Connection Lines between stars (Skipped during rapid scroll for 0 scroll lag)
      if (!isScrolling) {
